...
use crate::{
    derive_session_id, Encoding, NargDeserialize, ProverState, StdHash,
    VerificationError, VerifierState,
};

fn test_session_id(tag: &[u8]) -> [u8; 32] {
    derive_session_id(tag)
}
...
fn seeded_prover_rng_is_deterministic_and_mixing_diverges() {
    ...
    // Mixing entropy changes the stream.
...
/// `DeriveSessionID` must match the draft's construction:
/// SHAKE128 over `"irtf-cfrg-fiat-shamir/session-id" || zeros(136) || tag`.
#[test]
fn session_id_matches_draft_construction() {
    ...
    assert_eq!(derive_session_id(b"discrete_logarithm"), expected);
}

/// The prover's transcript initialization must match the draft's formula:
/// every verifier message is the XOF over
/// `session_id || zeros(136) || encode(instance) || ...`.
#[test]
fn transcript_initialization_matches_manual_shake128() {
    let session_id = test_session_id(b"discrete_logarithm");
    let instance = [42u32, 7u32];
    ...
    shake.update(&[0u8; 136]);
    shake.update(instance.encode().as_ref());
    let mut reader = shake.finalize_xof();
    let mut expected = [0u8; 32];
    reader.read(&mut expected);
...
fn closure_codecs_round_trip() {
    // A foreign type with no Encoding/NargSerialize impls.
    struct Foreign(u64);
    ...
    let mut prover = ProverState::<StdHash>::new(&session_id, &instance);
    prover.prover_message_with(&value, |v| v.0.to_le_bytes());
    let prover_challenge: [u8; 16] = prover.verifier_message();
    let proof = prover.narg_string().to_vec();

    let mut verifier = VerifierState::<StdHash>::new(&session_id, &instance, &proof);
    let read = verifier
        .prover_message_with(|buf| {
            if buf.len() < 8 {
