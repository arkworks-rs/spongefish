...
use crate::{
    ...
    transcript::{
        self, Hierarchy, Interaction, InteractionPattern, Kind, Label, Length, Pattern as _,
        TranscriptPlayer,
    },
...
impl<H, U, R> transcript::Prover for ProverState<H, U, R>
where
    U: Unit,
    H: DuplexSpongeInterface<U>,
    R: RngCore + CryptoRng,
...
mod tests {
    ...
    use super::*;
    use crate::{codecs::unit::*, transcript::*};

    /// Test all operations in UnitPattern.
    #[test]
    fn test_prover_state_unit_pattern() {
    ...
    fn test_drop_unfinalized_panics() {
        ...
        let mut prover: ProverState = pattern.into();
        prover.message_units("data", &[1, 2, 3, 4]);
        // Dropping unfinalized prover state should panic
    }

    #[test]
    #[should_panic]
    fn test_wrong_message_panics() {
        let mut pattern = PatternState::<u8>::new();
        pattern.message_units("data", 4);
        let pattern = pattern.finalize();

        let mut prover: ProverState = pattern.into();
        prover.message_units("wrong", &[1, 2, 3, 4]);
    }
    ...
    fn test_add_units_too_many_elements_should_panic() {
        ...
        let mut prover: ProverState = pattern.into();
        prover.message_units("message", &[1, 2, 3]);
    }

    #[test]
    #[should_panic(expected = "")]
    fn test_ratchet_panics_when_not_expected() {
        let mut pattern = PatternState::<u8>::new();
