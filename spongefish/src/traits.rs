...
pub trait MessagePattern<T> {
    ...
    fn message(&mut self, label: Label, ty: PhantomData<T>) -> Result<(), Self::Error>;
    fn message_sized(&mut self, label: Label, size: usize) -> Result<(), Self::Error>;
    fn message_dynamic(&mut self, label: Label) -> Result<(), Self::Error>;
}
...
pub trait MessageProver<T> {
    ...
    fn message_fixed(&mut self, label: Label, value: &[T]) -> Result<(), Self::Error>;

    /// A dynamic sized message of type [T]
    fn message_dynamic(&mut self, label: Label, value: &[T]) -> Result<(), Self::Error>;
}
...
pub trait MessageVerifier<T: ?Sized> {
    ...
    fn message_mut_ref(&mut self, label: Label, out: &mut T) -> Result<(), Self::Error>;
}

pub trait ChallengePattern<T> {
    type Error: Error;

    fn challenge(&mut self, label: Label, ty: PhantomData<T>) -> Result<(), Self::Error>;
    fn challenge_sized(&mut self, label: Label, size: usize) -> Result<(), Self::Error>;
    fn challenge_dynamic(&mut self, label: Label) -> Result<(), Self::Error>;
}
...
pub trait ChallengeProver<T> {
    ...
    fn challenge_out(&mut self, label: Label, out: &mut T) -> Result<(), Self::Error>;

    /// A fixed size challenge of type [T]
    fn challenge_fixed(&mut self, label: Label, value: &mut [T]) -> Result<(), Self::Error>;
    ...
    fn challenge_dynamic(&mut self, label: Label, value: &mut [T]) -> Result<(), Self::Error>;
    ...
    fn challenge_array<const N: usize>(&mut self, label: Label) -> Result<[T; N], Self::Error>
    where
        T: Default,
    {
    ...
    fn challenge_vec(&mut self, label: Label, size: usize) -> Result<Vec<T>, Self::Error>
    where
        T: Default,
    {
    ...
    fn challenge_dynamic_vec(&mut self, label: Label, size: usize) -> Result<Vec<T>, Self::Error>
    where
        T: Default,
    {
        let mut result = (0..size).map(|_| T::default()).collect::<Vec<_>>();
...
pub trait HintPattern<T> {
    ...
    fn hint_sized(&mut self, label: Label, size: usize) -> Result<(), Self::Error>;
    fn hint_dynamic(&mut self, label: Label) -> Result<(), Self::Error>;
}
...
pub trait HintProver<T> {
    ...
    fn hint_fixed(&mut self, label: Label, value: &[T]) -> Result<(), Self::Error>;

    /// A dynamic sized hint of type [T]
    fn hint_dynamic(&mut self, label: Label, value: &[T]) -> Result<(), Self::Error>;
}
...
pub trait HintVerifier<T: ?Sized> {
    ...
    fn hint_mut_ref(&mut self, label: Label, out: &mut T) -> Result<(), Self::Error>;
}
