...
- **Breaking:** `Decoding::Repr` must be a fixed-width array: its bound is `hybrid_array::AsArrayMut<U>` (a slice bound admitted a `Vec`, which decodes every message from zero bytes). `ByteArray<N>` is removed in favour of `hybrid_array::ArrayN<u8, N>`; the crate is re-exported as `spongefish::hybrid_array`, and the derived `Repr` is that type. Squeezed buffers are no longer wiped on drop.
